...
const SENDER_EMAIL = "alston@joinprox.com"; // Update with your verified domain email
...
export async function sendMessage(message: string, attachments: string[] = []): Promise<any> {
...
async function sendWelcomeEmail(email: string) {
  try {
    const response = await axios.post("https://api.resend.com/emails", {
      from: SENDER_EMAIL,
      to: email,
      subject: "Welcome to JoinProx!",
      html: `<h1>Welcome to JoinProx!</h1><p>We're excited to have you on board!</p>`,
    }, {
      headers: { 
...
// ✅ Function to Add Users to `auth.users`
export async function addUserToAuth(email: string) {
  try {
    ...
    if (userExists) {
      console.log(`User with email ${email} already exists in auth.users.`);
      return { success: true, message: "User already exists, no need to create a new one." };
    }

    // Step 2: Insert email into `public.waitlist`
    ...
    // Step 3: Create a new user in `auth.users`
    const { data: newUser, error: userError } = await supabase.auth.admin.createUser({
      email,
      email_confirm: true, // Auto-confirm email to trigger SMTP
      redirect_to: "https://joinprox.com"
    });

    if (userError) throw new Error(userError.message);

    // Step 4: Send Welcome Email via Resend API
    await sendWelcomeEmail(email);

    return { success: true, message: "User added & welcome email sent." };
  } catch (error) {
    console.error("Error in addUserToAuth:", error.message);
